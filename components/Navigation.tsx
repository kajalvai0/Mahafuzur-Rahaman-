...
import { Home, MessageCircle, FileText, AlertCircle, HeartHandshake, Menu } from 'lucide-react';
...
export const TopBar: React.FC<TopBarProps> = ({ logoImage, title, onMenuClick }) => {
  return (
    <div className="bg-emerald-800 text-white p-4 shadow-md sticky top-0 z-50 flex items-center justify-between">
      <div className="flex items-center gap-3">
        <button 
          onClick={onMenuClick}
          className="p-2 hover:bg-emerald-700 rounded-xl transition-colors"
        >
          <Menu size={24} />
        </button>
                ...
                return (
                    <button
                        ...
                        className={`relative -top-5 flex flex-col items-center justify-center w-14 h-14 rounded-full shadow-lg transition-transform active:scale-95 ${
                            isActive ? 'bg-emerald-800 text-white ring-4 ring-emerald-100' : 'bg-emerald-700 text-white'
                        }`}
                    >
                        <Icon size={24} />
                    </button>
            ...
            return (
                <button
                    ...
                    className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${
                        isActive ? 'text-emerald-800 font-bold' : 'text-slate-500 hover:text-emerald-700'
                    }`}
                >
                    <Icon size={20} />
